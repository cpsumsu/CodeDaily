...
@pytest.mark.parametrize("file_path", ["leetcode/2713. 矩阵中严格递增的单元格数.md"])
def test_2713(file_path):
    metadata_format(file_path)
...
@pytest.mark.parametrize("file_path", ["leetcode/522. 最长特殊序列 II.md"])
def test_522(file_path):
    metadata_format(file_path)
...
@pytest.mark.parametrize("file_path", ["leetcode/2288. 价格减免.md"])
def test_2288(file_path):
    metadata_format(file_path)
...
@pytest.mark.parametrize("file_path", ["leetcode/2748. 美丽下标对的数目.md"])
def test_2748(file_path):
    metadata_format(file_path)
