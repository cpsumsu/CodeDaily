...
void run_case() {
	...
	// sol is total + 1, since sol cover all the element from [1, sol - 1], so sol += A[i] -> the number cover all the element within [1, sol + A[i] - 1], we check if sol + A[i] is the required solution
	for (int i = 0; i < N; i++) {
		if (A[i] > sol) {
			cout << sol << '\n';
			return ;
